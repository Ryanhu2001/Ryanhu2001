...
kicker: "PAPER SHARING · AGENT ENVIRONMENT SYNTHESIS"
---

# Paper-sharing-2: 智能体环境合成的三条技术路线

**本次 Sharing 核心视角**：从「环境构建」切入，对比三条技术路线，重点拆解每篇的数据合成 Pipeline，看它们是如何解决「智能体训练环境不足」这个核心问题的。

1. **Qwen-AgentWorld · 模型模拟**：让语言世界模型学会环境动力学——推理即环境，corner-case 随手造
2. **CLI-Universe · 程序合成**：结构化分类法出蓝图，Docker 落地，多阶段可执行过滤保质量
3. **Agent-World · 真实挖掘 + 自进化**：从真实世界挖数据库与工具，让 agent 的失败驱动下一轮扩展

> **阅读口径**：第二至四部分沿三篇论文的问题、Pipeline 和结果展开；超出原文、用于连接三篇的内容会标注“我的理解”；第五部分的对比表与判断全部是我的综合。
...
![自制图解：三条环境合成路线在环境本质、保真度、扩展方式上的对比总览](assets/wiki/paper-sharing-2/three-routes-map.svg)

*自制图解：把上表画成一张总览。带着这三张卡片的印象读后面三部分，每一节讲的都是其中一张卡片的展开。*
...
![SFT 轨迹筛选流程](assets/wiki/paper-sharing-2/figure-01-sft-filtering.png)

*原图来自 Qwen-AgentWorld 论文：SFT 拒绝采样流程——10,250 个候选查询经独立评判与两两比较后保留 7,094 条。*
...
有了上述的 WorldModel 基础，我们可以有两种方式来训练 Agent RL：

![自制图解：Decoupled 环境模拟器与 Unified 智能体基础模型两种范式的对比](assets/wiki/paper-sharing-2/simrl-paradigms.svg)

*自制图解：左边把 LWM 当陪练（环境侧），右边把 LWM 并进策略模型本身（预热）。两条路的代表性战绩都标在图里。*

### 范式一：Decoupled（环境模拟器）

策略智能体和世界模型是分离的模型。Qwen-AgentWorld 作为独立的环境模拟器：Agent 生成 action，WorldModel 按 history + action 生成 observation，循环至 done 后由 Verifier 判成败给 reward，RL 更新 Agent。

> Sim RL的环境：WorldModel只能预测state，**init_state仍然需要提前合成**
> 
...
![自制图解：CLI-Universe 从 100% 候选到 33.6% 保留的五阶段过滤漏斗](assets/wiki/paper-sharing-2/cli-universe-funnel.svg)

*自制图解：每一层的宽度按保留率等比绘制——被丢弃的三分之二正是数据效率的来源。*
...
![CLI-Universe 失败模式分析](assets/wiki/paper-sharing-2/figure-02-failure-modes.png)

*原图来自 CLI-Universe 论文：九种失败模式的分布——前沿模型主要倒在验证侧，训练后的小模型失败转移到执行侧。*
...
**我的理解**：前两条路线各解决了问题的一半——Qwen-AgentWorld 用模拟解决环境的 Scale，CLI-Universe 用程序合成加多阶段过滤解决任务质量。

第三篇要回答的问题是：**能否大规模挖掘真实世界主题，自动构建 database + tools + tasks，并且让 agent 的失败反过来驱动下一轮环境扩展？=> 自迭代 Loop**
...
![Agent-World 自进化训练闭环](assets/wiki/paper-sharing-2/figure-03-agent-world-loop.png)

*原图来自 Agent-World 论文：环境-任务发现与持续自进化训练组成的闭环——训练诊断反馈到下一轮环境扩展。*
...
![Agent-World 环境规模统计](assets/wiki/paper-sharing-2/figure-04-agent-world-scale-a.png)

*原图来自 Agent-World 论文：1,978 个环境的领域分布。*

![Agent-World 工具与任务规模统计](assets/wiki/paper-sharing-2/figure-05-agent-world-scale-b.png)

*原图来自 Agent-World 论文：19,822 个工具与任务的规模统计。*
...
![Agent-World 主结果对比](assets/wiki/paper-sharing-2/figure-06-agent-world-results.png)

*原图来自 Agent-World 论文：23 个智能体基准上的主结果——8B / 14B 一致优于专有模型与环境扩展基线。*

- 在 23 个具有挑战性的智能体基准上，Agent-World-8B 和 14B 一致地优于强大的专有模型和环境扩展基线

- 揭示了合成环境数量、自进化轮次与下游智能体性能之间的缩放关系

![Agent-World 环境数量缩放趋势](assets/wiki/paper-sharing-2/figure-07-agent-world-scaling.png)

*原图来自 Agent-World 论文：环境数量从 10 到 2000 的缩放曲线，平均准确率 19.6% → 37.9%。*
...
> **我的理解**：本部分的对比维度、趋势判断与开放问题都是我自己的综合，不出自任何一篇原文。
